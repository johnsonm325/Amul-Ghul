...
**
**************************************************************************************/
int InputValid::isInteger()
{
	tries = 0;
	do
	{
		counter = 0;	// variable to count the number of non integer digits
		negCounter = 0;
		...
		// Loops through each character and
		for (size_t count = 0; count < input.length(); count++)
		{
			// If it's not a digit
			if (!(isdigit(input[count])))
		...
		// While the counter is less than 0, this loop will continue to run.
	} while (counter > 0 || negCounter > 1);
